      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-5 py-10 lg:grid-cols-[270px_1fr]">
        ...
        <div className="space-y-8">
          <section className="rounded-[2rem] bg-gradient-to-br from-slate-900 to-slate-700 p-8 text-white shadow-xl">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 text-lg leading-8 text-slate-200">
                Guia base para criação de projetos de importação usando C#, EF
                Core, Scaffold-DbContext, leitura CSV, origem Firebird quando
                necessário, destino Comercial em MySQL e destino Plus em SQL
                Server.
              </p>
            </div>
          </section>
          ...
          <Section
            id="visao-geral"
            icon={Database}
            title="1. Visão geral do padrão"
          >
            <p>
              O padrão do projeto é manter uma única classe{" "}
              <strong>Importador.cs</strong> com métodos separados por entidade,
              como <strong>Cliente()</strong>, <strong>Produto()</strong>,{" "}
              <strong>Portador()</strong> e<strong>GradeProduto()</strong>. O{" "}
              <strong>Program.cs</strong> fica responsável por criar as conexões
              e entregar os contextos prontos para o importador.
            </p>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <SystemCard
                ...
                status="Documentado"
              >
                Usado nas importações para o banco Comercial. Mantém Pomelo,
                MySQL e senha padrão <strong>genesysjp</strong>
                nos exemplos.
              </SystemCard>
              <SystemCard
                title="Plus"
                type="Destino SQL Server"
                status="Foco atual"
              >
                Usado nas importações para SQL Server. Usuário padrão{" "}
                <strong>sa</strong> e senha <strong>genesysjp</strong>
                nos exemplos.
              </SystemCard>
              <SystemCard title="Velo" type="SQL Server" status="Reservado">
                Aba reservada para documentação futura. Por enquanto, manter
                vazio ou somente com placeholder.
              </SystemCard>
          ...
          Nome sugerido: ImportacaoClienteX`}</CodeBlock>
            <p>
              O padrão recomendado é usar <strong>.NET 8</strong>, mantendo os
              pacotes do EF Core na versão 8.x para evitar conflito de versão.
            </p>
          </Section>
          ...
          <Section
            id="estrutura"
            icon={FolderTree}
            title="3. Estrutura padrão do projeto"
          >
            <p>
              Use uma estrutura simples, separando origem e destino quando
              houver banco de origem.
            </p>
          ...
          └── Program.cs`}</CodeBlock>
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
              <div className="flex gap-3">
                ...
                <p>
                  Quando a origem for CSV, não precisa criar pasta{" "}
                  <strong>Origem</strong>. Nesse caso, o importador recebe
                  somente o contexto do destino.
                </p>
              </div>
            </div>
          </Section>
          ...
          <Section
            id="comercial"
            icon={Server}
            title="4. Aba Comercial - MySQL"
          >
            <p>
              Essa aba é para importações cujo destino é o banco{" "}
              <strong>Comercial</strong>, usando MySQL. A senha padrão nos
              exemplos já está definida como <strong>genesysjp</strong>.
            </p>

            <h3 className="text-lg font-bold text-slate-900">
              Pacotes NuGet - Comercial MySQL
            </h3>
          ...
          Install-Package Microsoft.Extensions.Configuration.FileExtensions -Version 8.0.0`}</CodeBlock>
            ...
            <h3 className="text-lg font-bold text-slate-900">
              Scaffold do destino Comercial
            </h3>
            <CodeBlock>
              {`Scaffold-DbContext
          "Server=localhost;Database=virgem10;User=root;Password=genesysjp;Port=3366;Default Command Timeout=1000;"
          Pomelo.EntityFrameworkCore.MySql -OutputDir Destino -Context DestinoContext -Force -NoOnConfiguring`}
            </CodeBlock>

            <h3 className="text-lg font-bold text-slate-900">
              Program.cs - CSV para Comercial
            </h3>
          ...
          var optionsDestino = new DbContextOptionsBuilder<DestinoContext>()
            ...
            <h3 className="text-lg font-bold text-slate-900">
              Program.cs - Firebird para Comercial
            </h3>
            <CodeBlock>{`using Microsoft.EntityFrameworkCore;
          using NomeDoProjeto.Destino;
          ...
          <Section id="plus" icon={Database} title="5. Aba Plus - SQL Server">
            <p>
              Essa aba é para importações cujo destino é o banco{" "}
              <strong>Plus</strong>, usando SQL Server. Nos exemplos, o usuário
              padrão é <strong>sa</strong> e a senha é{" "}
              <strong>genesysjp</strong>.
            </p>

            <h3 className="text-lg font-bold text-slate-900">
              Pacotes NuGet - Plus SQL Server
            </h3>
            <CodeBlock>{`Install-Package Microsoft.EntityFrameworkCore -Version 8.0.8
          Install-Package Microsoft.EntityFrameworkCore.Design -Version 8.0.8
          Install-Package Microsoft.EntityFrameworkCore.Tools -Version 8.0.8
          Install-Package Microsoft.EntityFrameworkCore.SqlServer -Version 8.0.8`}</CodeBlock>
            ...
            <h3 className="text-lg font-bold text-slate-900">
              Scaffold do destino Plus
            </h3>
            <CodeBlock>{`Scaffold-DbContext "Server=localhost\\SQLEXPRESS;Database=BancoPlusDestino;User
          Id=sa;Password=genesysjp;TrustServerCertificate=True;" Microsoft.EntityFrameworkCore.SqlServer -OutputDir
          Destino -Context DestinoContext -Force -NoOnConfiguring`}</CodeBlock>

            <h3 className="text-lg font-bold text-slate-900">
              Scaffold da origem SQL Server, quando existir banco origem
            </h3>
            <CodeBlock>{`Scaffold-DbContext "Server=localhost\\SQLEXPRESS;Database=BancoOrigem;User
          Id=sa;Password=genesysjp;TrustServerCertificate=True;" Microsoft.EntityFrameworkCore.SqlServer -OutputDir
          Origem -Context OrigemContext -Force -NoOnConfiguring`}</CodeBlock>

            <h3 className="text-lg font-bold text-slate-900">
              Program.cs - SQL Server para SQL Server
            </h3>
            <CodeBlock>{`using Microsoft.EntityFrameworkCore;
          ...
          @"Server=localhost\\SQLEXPRESS;Database=BancoOrigem;User
          Id=sa;Password=genesysjp;TrustServerCertificate=True;";

          string connectionDestino =
          @"Server=localhost\\SQLEXPRESS;Database=BancoPlusDestino;User
          Id=sa;Password=genesysjp;TrustServerCertificate=True;";
          ...
          <Section id="velo" icon={FileText} title="6. Aba Velo - reservado">
            <p>
              Aba reservada para documentação futura das importações para o
              Velo.
            </p>
          ...
          <Section
            id="csv-exemplos"
            icon={Terminal}
            title="8. Importador.cs - exemplos CSV"
          >
            <p>
              Classe única <strong>Importador.cs</strong> com construtores para
              cenários com apenas destino ou com origem e destino.
            </p>
            <CodeBlock>{`using Microsoft.EntityFrameworkCore;
          using NomeDoProjeto.Destino;
          ...
          private static List<string[]> GetCSV(string caminho)
            ...
            {
            List<string[]> grades = GetCSV(@"\\kailon\IMPORTACAO\Importações Victor\Mix More - sistema
              kigi\backup\produtosGrade.csv");
          ...
          <Section
            id="firebird-exemplos"
            icon={Wrench}
            title="9. Importador.cs - exemplos Firebird"
          >
            <p>
              Exemplo mantendo o padrão de leitura por{" "}
              <strong>BancoOrigem.Execute()</strong>. A configuração da conexão
              Firebird fica fora desta documentação.
            </p>
          ...
          <Section
            id="boas-praticas"
            icon={CheckCircle2}
            title="10. Boas práticas para importações"
          >
            ...
            <p>
              Exemplo de carregamento rápido em memória para evitar consulta
              dentro do foreach:
            </p>
